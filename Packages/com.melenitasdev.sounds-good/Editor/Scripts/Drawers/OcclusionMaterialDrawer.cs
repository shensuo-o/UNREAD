...
{
    [CustomPropertyDrawer(typeof(OcclusionMaterial))]
    public class OcclusionMaterialDrawer : PropertyDrawer
    {
        // ----- Fields
        private string[] names;
        private OcclusionMaterial[] values;
        private bool cached;
        ...
        private void Cache ()
        {
            ...
            var fields = typeof(OcclusionMaterial)
                .GetFields(BindingFlags.Public | BindingFlags.Static)
                .Where(f => f.FieldType == typeof(OcclusionMaterial))
                .OrderBy(f => f.Name == "Null" ? 0 : 1)
                .ThenBy(f => f.Name);

            names = fields.Select(f => f.Name).ToArray();
            values = fields.Select(f => (OcclusionMaterial)f.GetValue(null)).ToArray();

            if (names.Length == 0)
            {
                names = new[] { "-----" };
                values = new[] { new OcclusionMaterial(string.Empty) };
            }
